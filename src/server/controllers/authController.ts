...
export const register = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!confirmPassword) {
      res.status(400).json({ success: false, message: 'Please confirm your password' });
      return;
    }
    ...
    const existing = await StorageProvider.findUserByEmail(email);
    if (existing) {
      res.status(409).json({
        success: false,
    ...
    // 4. Create user
    const newUser = await StorageProvider.createUser({
      fullName,
      mobileNumber,
      email,
      passwordHash,
...
export const login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const user = await StorageProvider.findUserByEmail(email);
    if (!user) {
      res.status(401).json({
        success: false,
